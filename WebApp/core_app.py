...
def load_vgg16_model():
  ...
  vgg_model.load_weights("static/weights/vgg16_weights_tf_dim_ordering_tf_kernels.h5")
  return vgg_model
  ...
  if request.method == "POST":
    ...
    if 'img' in request.files:
      ...
      if(type_=="mal" or type_=='brain'):
        test_image = load_img(name, target_size=(128, 128))
        ...
        # test_image = resize_image_pnm(test_image)                      #Resize the images to 128x128 pixels

        test_image = np.array(test_image)  
        test_image = resize_image_pnm(test_image)                            #Convert the image to numpy array
      ...
      if(type_=='mal'):
         ...
         preds, pred_val = translate_pnm(model["model"].predict(data))   #change
         final_json.append({"empty": False, "type":model["type"], 
                            "bac":preds[0], 
                            "normal":preds[1],
                            "viral":preds[2],
